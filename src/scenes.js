/**
 * Zano Quiz - Scene Definitions for Quiz Creation Wizard
...
const quizCreationScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      if (
        parsedQuestion.questionText.length <
        QUIZ_CONSTRAINTS.QUESTION_MIN_LENGTH
      ) {
        await ctx.reply(
      ...
      if (
        parsedQuestion.questionText.length >
        QUIZ_CONSTRAINTS.QUESTION_MAX_LENGTH
      ) {
        await ctx.reply(
      ...
      await ctx.reply(
        `${UI.COLORS.ERROR} An error occurred while processing your question.`
      );
      return ctx.scene.leave();
    }
  }
