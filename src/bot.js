...
function registerHandlers() {
  ...
  bot.telegram.setMyCommands([
    ...
    { command: 'create_quiz', description: 'Create a new quiz using the wizard' },
    { command: 'start_quiz', description: 'Start a quiz in a group (admin only)' },
...
function init() {
  try {
    ...
    bot.catch((err, ctx) => {
      ...
      logger.error(`Error in bot update [${updateType}] for chat ${chatId}, user ${userId}:`, err);
      
      // Notify user about the error if possible
      if (ctx.chat && ctx.chat.id) {
        ctx.reply(`${handlers.UI.COLORS.ERROR} An error occurred while processing your request. Please try again later.`)
